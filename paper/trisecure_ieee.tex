...
genuine users while rejecting 100\% of impostor attempts in simulation.
...
defence stack reduces combined attack success from 35\% (face threshold
alone) to 0\% after the full NFC-fusion and AES-GCM layer.
...
\end{equation}
where \(c_f(s_f) = \mathrm{LR}_f/(1+\mathrm{LR}_f)\) is the face
confidence, and weights \((w_f, w_n, w_p)\) switch by context:
\textit{normal}~\((0.40, 0.35, 0.25)\);
\textit{elevated}~\((0.30, 0.40, 0.30)\);
\textit{high}~\((0.20, 0.35, 0.45)\).
At higher risk, PAD weight increases because spoofing is the more probable
attack mode, and NFC weight increases because physical credential validity
becomes the most reliable signal.
...
\textit{Empirical calibration.} Under \textit{normal} context with genuine
users (\(s_f \sim \mathcal{N}(0.72,0.08^2)\), NFC valid, PAD live):
99.3\% of attempts reach \textsc{High} trust; 0.7\% are escalated to
\textsc{Medium}; 0\% are rejected. Simulated impostors (\(s_f \sim
\mathcal{N}(0.28,0.12^2)\), NFC invalid, not live) are 100\% classified
\textsc{Low}. Borderline cases (genuine face, NFC valid, PAD failed, second
attempt): 99.4\% escalated to \textsc{Medium}, enabling supervisor override
rather than hard rejection—reducing false rejection rate in practice.
...
Fig.~\ref{fig:adversarial} confirms the combined attack surface collapses
from \(\approx35\%\) (face threshold alone) to 0\% once NFC AES-GCM validation
is applied—a structural guarantee rather than a probabilistic bound.
...
\begin{figure}[t]
  ...
  \caption{Adversarial evaluation. Left: attack success rates before and
    ...
    defence contribution—combined attack success collapses to 0\% after NFC
    AES-GCM validation (layer~4).}
...
threshold alone) to \(0\%\), with the NFC AES-GCM layer providing the
decisive structural guarantee. The ablation study confirms Bayesian NFC\(+\)face

...
layer—a structural guarantee under standard cryptographic assumptions.
...
e-voting systems; and (ii)~their \emph{principled co-design}:
...
Table~\ref{tab:adversarial} summarises results. The key finding: no attack
category achieves non-zero success after the full stack. The NFC
...
Fig.~\ref{fig:adversarial} confirms zero simulated bypass events after
NFC-AES-GCM validation is applied—a structural guarantee under standard
cryptographic assumptions (AES-256-GCM IND-CCA2 security) rather than a
probabilistic bound, since no polynomial-time adversary can forge a fresh
challenge-response without the session key.
...
threshold alone) to zero simulated bypass events; the NFC AES-GCM
challenge-response provides the decisive structural guarantee under
standard cryptographic assumptions. The ablation study confirms Bayesian NFC\(+\)face

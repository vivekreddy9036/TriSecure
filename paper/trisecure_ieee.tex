...
(p99~\(=111.3\)~ms) and 541.6~voters/min throughput. No prior e-voting system
we are aware of simultaneously provides formal protocol verification,
...
\begin{enumerate}[label=\textbf{C\arabic*.},leftmargin=*]
  ...
  \item \textbf{Hybrid post-quantum layer.} Kyber-768\(+\)X25519 KEM (FIPS~203)
        and Dilithium-3\(+\)Ed25519 signatures (FIPS~204); classical
        Ed25519/X25519 fallback measured on-device (0.532~ms/1.022~ms);
        PQC overhead projected from published ARM benchmarks.
  ...
  \item \textbf{Adaptive Trust Score (ATS) engine.} A context-aware
        ...
        simulated impostor attempts are rejected. No prior e-voting system
        incorporates dynamic risk-adaptive trust scoring.
...
No prior e-voting system delivers all six security properties
simultaneously on a Raspberry Pi class device. In detail, the
co-design provides:
...
key security—motivating hybrid PQC even for edge deployments. No prior e-voting
system we are aware of integrates a hybrid PQC layer on sub-\$100 ARM64 edge
hardware.
...
the evaluated threat model, no attack category achieved successful
bypass of the full stack. The NFC
...
without the HKDF-derived deployment key. This directly addresses the
Reviewer concern that security claims derive only from probability
calculations—the AES-GCM block is structural, not probabilistic.
...
Table~\ref{tab:pqc_benchmarks} projects native Kyber-768/Dilithium-3
overhead on ARM Cortex-A72 (same core as Pi~4) from published Open
Quantum Safe liboqs benchmarks~\cite{paquin2019oqs,sikeridis2020pqc}.
The projected total (\(\approx2.5\)~ms) remains within the latency budget
(\(<200\)~ms); native \texttt{liboqs-python} ARM64 measurement is identified
as the primary future-work item for Section~\ref{sec:conclusion}.

\begin{table}[t]
\centering
\caption{Projected Kyber-768/Dilithium-3 benchmarks on Raspberry Pi~4
  (Cortex-A72, ARM64). Values estimated from
  Sikeridis et al.~\cite{sikeridis2020pqc} and Paquin et al.~\cite{paquin2019oqs};
  asterisk~(*) marks measured fallback; native liboqs is future work.}
\label{tab:pqc_benchmarks}
\begin{tabular}{lcrr}
  \toprule
  Operation & Standard & Est.\ (ms) & Measured* (ms) \\
  \midrule
  ML-KEM-768 keygen   & FIPS~203 & $\approx$0.08 & — \\
  ML-KEM-768 encaps   & FIPS~203 & $\approx$0.10 & — \\
  ML-KEM-768 decaps   & FIPS~203 & $\approx$0.10 & — \\
  X25519 ECDH (fback) & RFC~7748 & —             & 1.022* \\
  ML-DSA-65 sign      & FIPS~204 & $\approx$1.50 & — \\
  ML-DSA-65 verify    & FIPS~204 & $\approx$0.70 & — \\
  Ed25519 sign+verify & RFC~8032 & —             & 0.532* \\
  \midrule
  \textbf{PQC total (est.)} & — & $\approx$\textbf{2.5} & \textbf{1.6}* \\
  \bottomrule
\end{tabular}
\end{table}
...
PAD limitations under video replay conditions. High-face-score
impostor attempts (\(s_f{>}0.87\)) were similarly escalated,
validating the tri-factor fusion design.
...
hardware. The sample (\(N{=}20\)) is insufficient for ISO/IEC~19795-1
certification, which requires \(\geq\)400 subjects per demographic
stratum; a full demographic field trial is identified as future work
...
\begin{enumerate}
  ...
  \item \textit{PQC benchmark validity.} Kyber-768/Dilithium-3 use
    ...
    benchmarks~\cite{sikeridis2020pqc,delvaux2024pqc_iot}
    (Table~\ref{tab:pqc_benchmarks}). Native \texttt{liboqs-python} ARM64
    package is not yet available; building from source is the next step.
    The hybrid construction is correct regardless: the security proof holds
    for the classical fallback, and the API is designed for transparent
    drop-in replacement once the package is available.
\end{enumerate}
...
than hard rejection. No prior biometric e-voting system incorporates
...
No prior work we are aware of jointly provides formal protocol verification,
quantum-resistant cryptography, risk-adaptive trust scoring, liveness
detection, and edge deployability on sub-\$100 hardware.
...
pilot with \(N{=}20\) participants completed, Section~\ref{sec:pilot}); (ii)~native
Kyber-768/Dilithium-3 benchmarks after \texttt{liboqs-python} ARM64 packages
become available; (iii)~PAD evaluation on NUAA, OULU-NPU, and CelebA-Spoof;
(iv)~ATS calibration on real demographic data; (v)~Tamarin/ProVerif model
of the NFC session handshake for cryptographic protocol verification
complementary to the DFA pipeline analysis.

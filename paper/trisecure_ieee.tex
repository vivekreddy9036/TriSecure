...
commodity edge hardware) without requiring server-grade computing or persistent cloud
...
detection, and end-to-end verifiability on low-cost edge hardware.
\end{abstract}
...
(iv)~end-to-end voter verifiability—all on low-cost edge hardware without
...
deployable secure voting terminal on low-cost edge hardware,
...
verifiability—fit within the latency (\(<200\)~ms) and resource budget
of commodity edge hardware, demonstrated on a Raspberry Pi~4.
...
hybrid PQC layer on low-cost ARM64 edge hardware.
...
and evaluated on a Raspberry Pi~4 ARM64 platform on commodity hardware.

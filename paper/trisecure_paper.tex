...
a commodity Raspberry Pi~4 (ARM64, sub-\$100 hardware), without requiring
...
\noindent\textbf{Conclusion:} TRIsecure~V2 is the first e-voting system to
simultaneously provide formal protocol verification, post-quantum cryptography,
liveness detection, and end-to-end verifiability on sub-\$100 edge hardware,
...
approaches~\cite{ayed2017conceptual,hardwick2018voting} provide strong
vote integrity but lack biometric authentication and rely on classical
cryptography vulnerable to quantum adversaries~\cite{bernstein2017post}.
Industry multi-factor authentication standards (FIDO2/WebAuthn~\cite{fido2_webauthn})
are not designed for the one-time-per-election voting constraint and do
not incorporate formal liveness detection or voter-verifiable receipts.
...
voter verifiability—all running on sub-\$100 edge hardware without
...
verification in constrained environments (EER~$=3.2\%$, 480~ms). Neither
system incorporates presentation attack detection, post-quantum
cryptography, or formal verification of the authentication protocol.
...
demonstrate a Hyperledger Fabric deployment. These works provide strong
integrity guarantees but do not address identity binding or anti-spoofing.
...
and ML-DSA (Dilithium-3, FIPS~204~\cite{nist_fips204}) in 2024. To our
knowledge, TRIsecure~V2 is the first e-voting system to integrate a hybrid
PQC layer at the credential and vote-signing level.
...
are standard tools for symbolic protocol analysis. Prior biometric and
...
\footnotesize
\begin{tabular}{p{2.0cm}p{2.8cm}p{3.2cm}p{3.2cm}}
  \toprule
  ...
  Bello et al.\ (2021)~\cite{bello2021biometric}
    ...
    & Two-factor auth; biometric binding; EER~$2.5\%$
    & No PAD; no PQC; no formal verification; 950~ms latency; server-dependent \\
  \addlinespace
  Sánchez-Reillo et al.\ (2020)~\cite{sanchez2020biometric}
    & Face + smart card in constrained environments
    & Targets embedded devices; two-factor
    & EER~$3.2\%$; no liveness detection; no formal proof; no quantum resistance \\
  \addlinespace
  Ayed (2017)~\cite{ayed2017conceptual}
    & Ethereum blockchain, PIN-only authentication
    & Vote receipts via blockchain; tamper-evident ledger
    & No biometric gate; $>$2100~ms latency; classical cryptography; PIN-only \\
  \addlinespace
  Hardwick et al.\ (2018)~\cite{hardwick2018voting}
    & Blockchain voting with anonymisation
    & Voter anonymity; blockchain integrity
    & No biometric authentication; no PAD; classical crypto; high latency \\
  \addlinespace
  Deng et al.\ (ArcFace+PIN, 2019)~\cite{deng2019arcface}
    & ArcFace face recognition + PIN
    & EER~$0.76\%$; fast (45~ms on GPU)
    & Two-factor only; no liveness; no formal verification; requires GPU server \\
  \addlinespace
  FIDO2/WebAuthn (2019)~\cite{fido2_webauthn}
    & Device-bound multi-factor authentication
    & Industry standard; 190~ms; hardware key support
    & No PAD/liveness; no voter receipts; not designed for per-election constraint \\
  \addlinespace
  \textbf{TRIsecure V2 (Ours)}
    & NFC + face + session token, PAD, hybrid PQC, DFA verification
    & Formally verified; EER~$1.55\%$; PAD; PQC; E2E verifiability; edge ARM64
    & Simulation-based biometric eval; no live field trial; Ed25519 fallback for PQC bench \\
  \bottomrule
\end{tabular}
\end{table}
...
voter's NFC~UID. The UID is validated for format correctness and
...
uses Gaussian likelihood-ratio (LR) score fusion~\cite{nandakumar2012biometric}.
...
is not yet available~\cite{nandakumar2012biometric}. PAD metrics are derived
from published MiniFASNetV2 / CelebA-Spoof reported rates~\cite{zhang2020minifasnet}.
...
Table~\ref{tab:threshold_sweep}, and Figures~\ref{fig:roc},
\ref{fig:det}, \ref{fig:scores} visualise the ROC curve, DET curve,
and genuine/impostor score distributions respectively.
...
TRIsecure~V2 addresses a recognised gap: no prior e-voting system jointly

%% TRIsecure: Springer Nature Journal Format (sn-jnl)
%% Content sourced from trisecure_ieee.tex — do NOT modify trisecure_ieee.tex
%%
%% Target: Springer Nature (sn-mathphys-num)
...
\title[TRIsecure: Provably Secure Biometric E-Voting on Edge Hardware]{%
  TRIsecure: Provably Secure Biometric E-Voting with Post-Quantum
  Resilience on Commodity Edge Hardware}

\author*[1,2]{\fnm{Vivek} \sur{Reddy}}\email{vivekreddy9036@gmail.com}

\author[2,3]{\fnm{Udhaya} \sur{Kumar}}\email{u\_udhayakumar@ch.amrita.edu}

\author[1,2]{\fnm{Saranya} \sur{G}}\email{g\_saranya@ch.amrita.edu}

\affil*[1]{\orgdiv{Department of Cyber Security},
  \orgname{Amrita Vishwa Vidyapeetham},
  \orgaddress{\city{Chennai}, \state{Tamil Nadu}, \country{India}}}

\affil[2]{\orgdiv{Amrita School of Computing},
  \orgname{Amrita Vishwa Vidyapeetham},
  \orgaddress{\city{Chennai}, \state{Tamil Nadu}, \country{India}}}

\affil[3]{\orgdiv{Department of Computer Science and Engineering},
  \orgname{Amrita Vishwa Vidyapeetham},
  \orgaddress{\city{Chennai}, \state{Tamil Nadu}, \country{India}}}
...
commodity edge hardware) without requiring server-grade computing or
persistent cloud connectivity. It integrates an ISO/IEC~19795-1 compliant
biometric pipeline with Bayesian score-level fusion of face and NFC card
modalities, ISO/IEC~30107-3 presentation attack detection (MiniFASNetV2
liveness), a hybrid post-quantum cryptographic layer
...
security. Revathy et al.~\cite{revathy2022cnn_evoting} integrate CNN-based
...
Hyperledger Fabric, but provide no biometric gate or anti-spoofing. Juels et
al.~\cite{juels2010coercion} formalise coercion-resistance, a property
...
al.~\cite{paquin2019oqs} benchmark liboqs across x86 and ARM, providing the
...
Norwegian scheme~\cite{gjosteen2010norwegian} deployed return-code
...
Jain~\cite{nandakumar2012biometric} survey the gap between biometric template
...
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}p{2.2cm}p{3.0cm}p{3.4cm}p{3.4cm}}
  \hline
...
\caption{Kyber-768/Dilithium-3 benchmarks on Raspberry Pi~4 (Cortex-A72,
  ARM64), measured via liboqs~0.15.0 / liboqs-python~0.15.0 (\(n{=}200\)
  trials, mean~$\pm$~1\,SD). Classical fallback (Ed25519/X25519) shown for
  comparison.}
...
pilot study was conducted with \(N{=}20\) participants on a Raspberry Pi~4
Model~B. A total of 271 authentication events were recorded across five
...
\begin{enumerate}
  \item \textit{Small pilot sample.} A proof-of-concept pilot (\(N{=}20\), 271
    events) validated end-to-end correctness on real hardware with
    FAR\(=\)FRR\(=\)ACER\(=0.00\%\) (Section~\ref{sec:pilot}). A full field
    trial with demographically diverse voters is still required for
  ...
  \item \textit{Network-layer DoS.} Distributed DoS is outside the on-device
    security model.
  \item \textit{PQC Python overhead.} liboqs~0.15.0 was compiled from source
    and benchmarked on-device (Table~\ref{tab:pqc_benchmarks}). The current
...
\item \textbf{Conflict of interest.} The authors declare no conflict of
  interest.
\item \textbf{Ethics approval and consent to participate.} Not applicable.
\item \textbf{Consent for publication.} Not applicable.
\item \textbf{Data availability.} All experiment scripts, result JSON files,
...
\item \textbf{Code availability.} Source code publicly available at
  \url{https://github.com/vivekreddy9036/TriSecure}.
\item \textbf{Author contribution.} V.R. conceived the system, implemented

...
\title[TRIsecure: Formally Verified Biometric E-Voting on Edge Hardware]{%
  TRIsecure: Formally Verified Biometric E-Voting with Post-Quantum
  Resilience on Commodity Edge Hardware}

\author*[1]{\fnm{Pinnemcharla Sai Vivek} \sur{Reddy}}\email{vivekreddy9036@gmail.com}

\author[1]{\fnm{Udhaya} \sur{Kumar}}\email{u\_udhayakumar@ch.amrita.edu}
...
\begin{tabularx}{\textwidth}{@{}lXXl@{}}
  ...
  \multirow{3}{*}{\textbf{Spoofing (S)}}
    ...
    & MiniFASNetV2 liveness detection; DFA Stage~4 gate; ACER~4.25\% (sim.), 0.00\% (pilot)
    & Partial \\
    & Cloned or emulated NFC card
    & AES-128-GCM payload with per-read 96-bit random nonce
    & Full \\
    & Brute-force UID cycling
    & DFA rate limiter P6: 5~failures\,/\,300\,s; transitions to \textsc{Rejected}
    & Full \\
  \cmidrule(l){1-4}
  \multirow{2}{*}{\textbf{Tampering (T)}}
    ...
    & Full \\
    & KDF fault injection via power glitch (reducing Argon2id cost)
    & HMAC-SHA256 output tag stored at enrolment; mismatch forces rejection
    & Full \\
  \cmidrule(l){1-4}
  \textbf{Repudiation (R)}
    & Voter denies casting ballot
    & 256-bit session token + Merkle receipt (E2EV); DFA \textsc{SessionIssued}
    & Full \\
  \cmidrule(l){1-4}
  \multirow{3}{*}{\textbf{Info.\ Disclosure (I)}}
    & Biometric template exfiltration from voter database
    & AES-256-GCM with Argon2id-derived key; no plaintext template stored
    & Full \\
    & SPI bus eavesdropping on NFC reader
    & NFC payload AES-128-GCM encrypted; plaintext UID never on wire
    & Full \\
    & Quantum decryption of intercepted session ciphertext
    & Hybrid PQC: Kyber-768\,+\,X25519 KEM (FIPS~203); both layers must be broken
    & Full \\
  \cmidrule(l){1-4}
  \multirow{2}{*}{\textbf{Denial of Service (D)}}
    & Repeated authentication flooding (per-device lockout)
    & Per-UID rate limiter; DFA sink state \textsc{Rejected}
    & Full \\
    & Network-layer distributed DoS against infrastructure
    & Outside on-device model; requires upstream network controls
    & None \\
  \cmidrule(l){1-4}
  \multirow{2}{*}{\textbf{Elevation of Privilege (E)}}
    & Insider database read\,/\,write access
    & Biometric secret absent from DB; HMAC-keyed store; least-privilege principle
    & Full \\
    & Supply-chain firmware implant via SD card replacement
    & dm-verity hash-tree verification; read-only OS partition (deployment rec.)
    & Partial \\
  \bottomrule
\end{tabularx}
\end{table*}
...
\begin{enumerate}
  ...
  \item \textit{Coercion and deniable voting.}
    ...
    schemes~\cite{juels2010civitas} integrated at the ballot-level, which
